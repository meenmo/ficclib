...
from ficclib.swap.curves.base import DiscountCurve
...
class OISDiscountCurve(DiscountCurve):
    ...
    def __init__(self, 
        ...
        sorted_pairs = sorted(zip(pillar_times, discount_factors, strict=False))
        ...
        if interpolator is not None:
            ...
            if interpolation_method in ["LINEAR_DF", "STEP_FORWARD_CONTINUOUS", "STEP_FORWARD"]:
                ...
                zero_rates = []
                for t, df in zip(pillar_times, discount_factors, strict=False):
                    if t > 0:
    ...
    def get_pillar_info(self) -> List[tuple[float, float, float]]:
        ...
        info = []
        for t, df in zip(self.pillar_times, self.discount_factors, strict=False):
